<template>
    <div class="audio-player">
        <div class="audio__btn-wrap">
            <div v-show="showPrevButton" class="audio__play--previous"
                 :class="{ disable: !isLoop && currentPlayIndex === 0 }" @click="playPrev">
                <svg class="audio__play__icon" aria-hidden="true">
                    <use xlink:href="#icon-play-prev"/>
                </svg>
                <!--                <img src="../../assets/previousMusic.svg"/>-->
            </div>
            ...
            <div v-show="showNextButton" class="audio__play--next"
                 :class="{ disable: !isLoop && currentPlayIndex === audioList.length - 1 }" @click="playNext">
                <svg class="audio__play__icon" aria-hidden="true">
                    <use xlink:href="#icon-play-next"/>
                </svg>
            </div>
        </div>
        <div v-show="showProgressBar" class="audio__progress-wrap" ref="audioProgressWrap"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 初始化音频进度的拖拽逻辑
            initProgressBarDrag () {
                this.$refs.audioProgressPoint.addEventListener('touchstart', event => {
                        // 设置拖拽中this.isDragging = true;
                    }, false
                );
                this.$refs.audioProgressPoint.addEventListener(
                    ...
                    event => {
                        ...
                        // 超出右边
                        if (touch.pageX > this.$refs.audioProgressWrap.offsetLeft + this.$refs.audioProgressWrap.offsetWidth) {
                            // 设置点点
                        ...
                        // 设置进度条
                        this.$refs.audioProgress.style.width = touch.pageX - this.$refs.audioProgressWrap.offsetLeft + 'px';
                        // 设置当前时间
            ...
            playing () {
                ...
                this.currentTimeAfterFormat = this.formatTime(this.$refs.audio.currentTime);
                // 设置播放进度条
                this.$refs.audioProgress.style.width = (this.$refs.audio.currentTime / this.$refs.audio.duration) * this.$refs.audioProgressWrap.offsetWidth + 'px';
                // 设置播放进度拖拽点
            ...
            playPrev () {
                ...
                let prevHandler = () => {
                    ...
                    this.$nextTick(() => {
                        this.play();
                        this.$emit('play-prev', this.currentPlayIndex);
                    });
                };
            ...
            playNext () {
                ...
                let nextHandler = () => {
                    ...
                    this.$nextTick(() => {
                        this.play();
                        this.$emit('play-next', this.currentPlayIndex);
                    });
                };
